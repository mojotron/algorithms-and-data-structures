...
=begin
  ...
  def bfs(start_value, end_value)
    start = search_vertex(start_value)
    queue = Array.new()
    queue << start
    while !queue.empty?
      temp_vertex = queue.shift()
      temp_edge = temp_vertex.edges.head
      until temp_edge == nil
        x = search_vertex(temp_edge.value)
        if x.color == 'white'
          x.distance = temp_vertex.distance + 1
          x.parent = temp_vertex.value
          x.color = 'gray'
          queue << x
        end
        temp_edge = temp_edge.link
